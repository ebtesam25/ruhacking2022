...
def getinfo(statename):
    ...
    print(statename)
    url = "https://us-central1-aiot-fit-xlab.cloudfunctions.net/aphrodite"
...
def sms_reply():
    ...
    flag = 0
    outstring = "Unfortunately Aphrodite did not understand the following message ..." + incoming
    ...
    if "is abortion legal in" in incoming:
        ...
        if info['status'] != "found":
            outstring= "Sorry, Aphrodite could not find information about! " + statename +  " please try again with a state name (spelling sensitive) "
            resp.message(outstring)
            return str(resp)
 
        outstring= "The current legal status of abortion in  " + statename +  " is " + info['legal status'] + " and this is in effect " + info['time of effect'] + "; Details: " + info['details'] + ". thank you for using Aphrodite"
        resp.message(outstring)
    ...
    if "how can i get funding" in incoming:
        ...
        outstring= "Please use the Aphrodite App to register a free account and submit a donation request. We will connect you to various organizations and private donors to fulfil your request. thank you for using Aphrodite"
        resp.message(outstring)
    ...
    # Add a message
    if flag ==0:
        outstring = "Unfortunately Aphrodite did not understand your message. Please message me again "
